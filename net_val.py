...
class DVSQ(object):
    ...
    def img_alexnet_layers(self, model_weights):
        ...
        distorted_image1 = tf.pack([tf.image.crop_to_bounding_box(tf.image.flip_left_right(each_image), 0, 0, height, width) for each_image in tf.unpack(reshaped_image)])
        distorted_image2 = tf.pack([tf.image.crop_to_bounding_box(tf.image.flip_left_right(each_image), 28, 28, height, width) for each_image in tf.unpack(reshaped_image)])
        distorted_image3 = tf.pack([tf.image.crop_to_bounding_box(tf.image.flip_left_right(each_image), 28, 0, height, width) for each_image in tf.unpack(reshaped_image)])
        distorted_image4 = tf.pack([tf.image.crop_to_bounding_box(tf.image.flip_left_right(each_image), 0, 28, height, width) for each_image in tf.unpack(reshaped_image)])
        distorted_image5 = tf.pack([tf.image.crop_to_bounding_box(tf.image.flip_left_right(each_image), 14, 14, height, width) for each_image in tf.unpack(reshaped_image)])

        distorted_image6 = tf.pack([tf.image.crop_to_bounding_box(each_image, 0, 0, height, width) for each_image in tf.unpack(reshaped_image)])
        distorted_image7 = tf.pack([tf.image.crop_to_bounding_box(each_image, 28, 28, height, width) for each_image in tf.unpack(reshaped_image)])
        distorted_image8 = tf.pack([tf.image.crop_to_bounding_box(each_image, 28, 0, height, width) for each_image in tf.unpack(reshaped_image)])
        distorted_image9 = tf.pack([tf.image.crop_to_bounding_box(each_image, 0, 28, height, width) for each_image in tf.unpack(reshaped_image)])
        distorted_image0 = tf.pack([tf.image.crop_to_bounding_box(each_image, 14, 14, height, width) for each_image in tf.unpack(reshaped_image)])
        
        distorted_image = tf.concat(0, [distorted_image1, distorted_image2, distorted_image3, distorted_image4, distorted_image5, distorted_image6, distorted_image7, distorted_image8, distorted_image9, distorted_image0])
        ...
        fc8_t = tf.concat(0, [tf.expand_dims(i, 0) for i in tf.split(0, 10, self.fc8_t)])
